...
-- the realm is stored in every case, even for a bare pseudo, or a transfer would re-root the entry. A key with no realm is not one entry: it is that
...
-- a Battle.net friend is never blocked, full stop. Only the tag used to be refused,
...
-- rectangle: the white rectangle once seen in the duration menu was U+25BE, the small
...
-- Decision 153: two clauses joined by a long dash read as machine-written, and
-- a comma, a colon or a full stop says
...
-- box, which was written that way on purpose.
--
...
-- Decision 162b: the two hairlines of the tab strip had different thicknesses. The strip carries ONE hairline along each edge, and three things
...
-- Decision 139, the long-rule option: a hairline of accent between two questions of
...
do
    ...
    -- section of muted grey was too faint, and this is the one line a dead
    ...
    -- The sentences themselves, to the letter: they are validated texts
...
-- of its own label. The validated sentence says what happens, for how long,
-- and how to undo it.
do
...
-- that is the one that came back no feedback at all: a BattleTag
...
do
    ...
    -- Finding 162a: the logo sat off-centre in the ring. `MiniMap-TrackingBorder` paints
...
-- could not be dragged past the horizontal -- the button could only travel the right half, 0 to 180 degrees, constat D.5.
...
-- larger than the default 80 falls inside the map -- the button sat inside the map instead of outside it.
...
-- And a PNJ gets nothing either, decision 113: the menu offered it on NPCs. Allowing or blocking a shopkeeper writes a dead record
...
-- The grip drives both dimensions, decision 98: the window only resized vertically. There is still no
...
-- The columns share the extra pixels between them. Every
...
-- pair used to record the current size unconditionally, which is why the
-- handle resized the window again at every re-click.
now = now + 5
...
-- Decision 162d: the default height ran too far under the "Test a name" field.
...
-- visible, working scrollbar, and step D.1 of the session sends the
-- tester down the home screen "with the wheel or the bar on the right": what the
-- bar does when it is pulled is a step of a manual session that costs three
