...
local C = {
    ...
    -- its background, which is exactly what the boxes were invisible, dark on dark describes. #262633 sits above both fills.
...
-- mock-up that was retained draws them at .55.
local WITNESS_ALPHA = 0.55

-- 15 / 14 / 13 / 12, the hierarchy that was asked for, thinned by one step at the
...
-- clicking the words of a box ticks the box (decision 135).
...
-- mark: an unticked box drew nothing and a ticked one a plain blue square. The harness cannot see this: its widgets answer every
...
-- and what the validated C2 mock-ups drew as discs. Three
...
local function newInput(parent, name, width, hintText, onEnter, keepText)
    ...
    -- ex. « test »" ran out of the box and under the Add button: the overflow that
    -- was reported. One line, and the wording
...
-- draws a white rectangle instead, which is what a white rectangle in the duration menu is. The font covers Latin-1 and the Windows-1252 additions and
...
-- through the right border: at the 500 px bound "Toto-Royaume
...
--
-- `ruleGap` is that air, and it is 24 on EACH side of the rule (decision 162c: the
-- air around the rule did not match the mock-up). The
...
-- it; half of it on each side is what made the screen read as a dense form next to the mock-up it came from.
--
...
local function buildProtectionTab(parent)
    ...
    -- Decision 139, the long-rule option: a hairline of accent between two
    ...
    -- too faint set among a whole section of muted grey, and it is the ONE
    ...
    -- is what made the window too tall -- and the fold is what a person
...
-- window empty under them, a half-height modal that had to be scrolled (finding
...
-- half is the whole of it, since this panel is not the one that blocks (decision 102).
...
local function layoutTabs()
    ...
    -- Decision 140, a bar across the top: the tabs sit INSIDE the window, in one
    ...
    -- decision 140 refused the rounded segment for not being faithful to the rest.
    --
...
local function createMainFrame()
    ...
    -- The title bar carries the two window gestures, decision 136 -- one drag behaviour everywhere. Dragging it moves the
    ...
    -- what without triggering a move on the way rests on: the client
    ...
    -- a click on the main window closes the drawer), which is the one
